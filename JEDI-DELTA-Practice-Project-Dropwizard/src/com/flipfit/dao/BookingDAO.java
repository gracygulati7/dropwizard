...
public class BookingDAO {
    ...
    private Booking mapResultSetToBooking(ResultSet rs) throws SQLException {
        ...
        booking.setCenterId(rs.getInt("centre_id"));
        Date slotDate = rs.getDate("slot_date");
        if (slotDate != null) {
            booking.setSlotDate(slotDate.toLocalDate());
        }
    ...
    public Booking createBooking(int userId, int slotId) {
        ...
        String sql = "INSERT INTO bookings (user_id, slot_id, centre_id, slot_date, start_time, end_time, status, is_deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {

            ps.setInt(1, userId);
            ps.setInt(2, slotId);
            ps.setInt(3, slot.getCenterId());
            ps.setDate(4, Date.valueOf(slot.getDate()));
            ps.setString(5, slot.getStartTime());
            ps.setString(6, slot.getEndTime());
            ps.setString(7, Booking.BookingStatus.CONFIRMED.name());
    ...
    public Booking createWaitlistingBooking(int userId, int slotId) {
        ...
        try (Connection conn = DBUtil.getConnection();
            ...
            ps.setString(5, slot.getStartTime());
            ps.setString(6, slot.getEndTime());
            ps.setString(7, Booking.BookingStatus.WAITLISTED.name());
            ps.setBoolean(8, false);
